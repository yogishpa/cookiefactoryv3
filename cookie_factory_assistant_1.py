...
async def start():
  ...
  # Safely access user_data
  event_id = None
  try:
    if hasattr(context.session, 'user_data'):
      print('user data', context.session.user_data)
      event_id = context.session.user_data.get('event_id')
  except AttributeError:
    print('No user_data attribute found')
    pass
...
async def on_action(action):
  ...
  res = await chain.acall({
    'event_title': event_title or "Unknown event",
    'event_description': event_description or "No description available",
    'event_timestamp': event_timestamp or "Unknown time"
  }, callbacks=[cb])
...
async def on_action(action):
  ...
  if action.value == "3d":
    ...
    await action.remove()
    # point camera to the entity id if available
    if event_entity_id:
